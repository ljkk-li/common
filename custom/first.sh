#!/bin/bash
# https://github.com/ljkk-li/build-actions
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/ljkk-li/common build/common
...
git clone -b main --depth 1 https://github.com/ljkk-li/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (ljkk-li/build-actions|ljkk-li/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/ljkk-li/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/ljkk-li/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="ljkk-li/build-actions"
  sudo rm -rf build common.sh
